...
## Phase 7B patient portal

The same application now includes patient activation/login, authenticator setup and recovery, patient reports/PDFs, security settings, access history, and public verification. Staff routes remain intact. The additional bundled runtime dependency is `qrcode.react` for local authenticator QR rendering.

See [Phase 7B documentation](../docs/PHASE_7B_PATIENT_FRONTEND.md) for patient routes, secret handling, existing PDF QR limitations, validation, and deployment. Use `/#/patient/login`, `/#/patient/activate`, or `/#/verify`. Existing PDF QR codes still target the unchanged verification API.
