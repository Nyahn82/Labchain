#!/usr/bin/env bash
set -euo pipefail
# Run on the single VPS. All four independent peer ledgers must converge.
...
output_dir="$NETWORK_DIR/artifacts/single-vps"
mkdir -p "$output_dir"
...
for attempt in {1..12}; do
  ...
  for number in 1 2 3 4; do
    target="peer$number"
    single_peer "$target"
    if ! peer channel getinfo -c "$CHANNEL" > "$work/node$number.info"; then success=false; continue; fi
    if [[ -n "$anchor_id" ]]; then
      payload="$(python3 -c 'import json,sys; print(json.dumps({"Args":["ReadAnchor",sys.argv[1]]}))' "$anchor_id")"
      if ! peer chaincode query -C "$CHANNEL" -n "$CHAINCODE" -c "$payload" > "$work/node$number.record"; then success=false; fi
    fi
  done
  if [[ "$success" == true ]] && python3 "$NETWORK_DIR/scripts/verify-observations.py" "$work" "$anchor_id" --single-vps; then
